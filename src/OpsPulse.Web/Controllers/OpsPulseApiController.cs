...
public class OpsPulseApiController : ControllerBase
{
    ...
    public async Task<IActionResult> GetHealthSummary()
    {
        ...
        return Ok(new
        {
            ...
            failedJobsLast24Hours = await _db.MaintenanceJobs.CountAsync(j => j.LastStatus == "Failed" && j.LastRunUtc >= cutoff),
    ...
    public async Task<IActionResult> GetOpenIncidents()
    {
        var rows = await _db.Incidents
            ...
            .Select(i => new { i.Id, i.Title, Site = i.Site == null ? "" : i.Site.Name, i.Severity, i.Status, i.DetectedAtUtc })
            .ToListAsync();
    ...
    public async Task<IActionResult> GetFailedJobs()
    {
        var rows = await _db.MaintenanceJobs
            ...
            .Select(j => new { j.Id, Site = j.Site == null ? "" : j.Site.Name, j.JobName, j.JobType, j.LastRunUtc, j.LastStatus, j.FailureMessage })
            .ToListAsync();
    ...
    {
        return Ok(await _db.AuditLogs.OrderByDescending(a => a.TimestampUtc).Take(25).ToListAsync());
    }
    ...
    public async Task<IActionResult> ExportFailedJobs()
    {
        ...
        {
            csv.AppendLine(string.Join(",", Csv(job.Site?.Name), Csv(job.JobName), Csv(job.JobType), Csv(job.LastRunUtc.ToString("O")), Csv(job.LastStatus), Csv(job.FailureMessage)));
        }
    ...
    {
        var incidents = await _db.Incidents.Include(i => i.Site).OrderByDescending(i => i.DetectedAtUtc).ToListAsync();
        var csv = new StringBuilder();
        csv.AppendLine("Site,Title,Severity,Status,DetectedAtUtc,ResolvedAtUtc,RootCause,ResolutionSummary");
        foreach (var i in incidents)
        {
            csv.AppendLine(string.Join(",", Csv(i.Site?.Name), Csv(i.Title), Csv(i.Severity), Csv(i.Status), Csv(i.DetectedAtUtc.ToString("O")), Csv(i.ResolvedAtUtc?.ToString("O")), Csv(i.RootCause), Csv(i.ResolutionSummary)));
        }
